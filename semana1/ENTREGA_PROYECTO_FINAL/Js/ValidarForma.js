...
function validainputs() {
    ...
    if (emailval === "") {
        ...
        onErrorform(email, "No ingreso / Email no valido");
        errorf = 1;
    } else {
        exitosform(email);
    }
    if (passwordval === "") {
        ...
        onErrorform(password, "No ingreso / Contraseña no válida");
        errorf = 1;
    } else {
        exitosform(password);
    }
...
/*
if (nombreval === "") {
    onErrorform(nombre, "Nombre no puede estar vacio");
} else {
    exitosform(nombre);
}
if (emailval === "") {
...
if (solicitudval === "") {
    onErrorform(solicitud, "Comentario o solicitud no puede estar vacio");
} else {
    exitosform(solicitud);
}
}
